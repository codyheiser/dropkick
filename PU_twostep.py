...
def twoStep(clf, X, y, n_iter=10, thresh='min'):
    ...
    if thresh=='min':
        ...
        label_thresh = [np.mean(pred[y==0])+np.std(pred[y==0]), np.mean(pred[y==1])-np.std(pred[y==1])]
    ...
    for i in range(n_iter):
        ...
        print('Step 1 labeled {} new positives and {} new negatives. {} unlabeled remaining.'.format(len(iP_new), len(iN_new), len(y[y==-1])))
